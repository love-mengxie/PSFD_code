...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 使用0号GPU
wm = "Paint-by-Example"
...
def main():
    ...
    image_folder = "/HARD-DATA2/ZHT/Paint-by-Example-main/imagenet/imagenet/imagenet/GT6"
    ref_folder = "/HARD-DATA2/ZHT/Paint-by-Example-main/imagenet/imagenet/imagenet/ref"
    mask_folder = "/HARD-DATA2/ZHT/Paint-by-Example-main/imagenet/imagenet/imagenet/mask"
    # image_folder = "./COCOEE/test_bench/GT_3500"
    # ref_folder = "./COCOEE/test_bench/Ref_3500"
    # mask_folder = "./COCOEE/test_bench/Mask_bbox_3500"
    attack_folder = "./adv/tar_imagenet005"
    edit_image_folder = "./results/imagenet"
    image_files = os.listdir(attack_folder)
    #image_files = image_files[1000:1300]
    files_num = len(image_files) - 1
    start_code = None
    ori_mse = ori_psnr = ori_ssim = ori_lp = 0
    edt_mse = edt_psnr = edt_ssim = edt_lp = 0
    if opt.fixed_code:
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                for image_file in image_files:
                    adv_image_path = os.path.join(attack_folder, image_file)
                    print(adv_image_path)
                    # image_path = os.path.join(image_folder, image_file, f"_GT.png")
                    filename = os.path.basename(adv_image_path)[:8]
                    if filename[0] != '0':
                        continue
                    image_path = os.path.join(image_folder, f"{filename}.JPEG")
                    ref_path = os.path.join(ref_folder, f"{filename}.JPEG")
                    mask_path = os.path.join(mask_folder, f"{filename}.png")
                    results_ori_path = os.path.join(edit_image_folder, f"{filename}.png")
                    # adv_image_path = os.path.join(attack_folder, image_file, f"{filename}.png")
                    # filename = os.path.basename(image_path)
                    img_p = Image.open(image_path).convert("RGB").resize((512, 512))
                    ...
                    results_ori = Image.open(results_ori_path).convert("RGB")
                    # image_path = opt.image_path
                    # img_adv = attack(pipe_img2img, image_path)

                    # print('original img and adversarial img')
                    mse, psnr, ssim, lp = evaluation(img_p, img_adv)
                    if psnr == float('inf'):
                        continue
                    print('mse=', mse)
                    print('psnr=', psnr)
                    ...
                    image_tensor = get_tensor()(img_adv)
                    ...
                    mse, psnr, ssim, lp = evaluation(img, results_ori)
                    if psnr == float('inf'):
                        continue
                    if ssim < min_ssim:
                        min_name = filename
