...
def main():
    ...
    # attack_folder = "./adv/new"
    attack_folder = "./COCOEE/test_bench/GT_3500"
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                for i in range(1):
                    ...
                    # filename = os.path.basename(adv_image_path)[:5]
                    filename = "000000006127"
                    #print(adv_image_path)
                    adv_image_path = os.path.join(attack_folder, f"{filename}_GT.png")
                    # adv_image_path = os.path.join(image_folder, f"{filename}_GT.png")
                    ...
                    if not opt.skip_save:
                        for i, x_sample in enumerate(x_checked_image_torch):
                            ...
                            img = put_watermark(img, wm_encoder)
                            num = num + 1
                            p = os.path.join(result_path, filename + ".png")
                            print(p)
                            img.save(p)
